...
void check_within_range(double lower, double upper, double val, int * results_success, char ** results_success_str, char * dwarf_name) {
//    * results_success = (lower < val) && (val < upper);
    size_t buff_len = sizeof(results_success_str);
//    if (* results_success)
//    {
        strncpy(results_success_str, dwarf_name, buff_len);
//    } else {
//        strncpy(results_success_str, "\t\t", buff_len);
//    }
    ...
    for (rho_bar_sub_c = 0.01; rho_bar_sub_c < 1.5; rho_bar_sub_c = rho_bar_sub_c + 0.01) {
        for (Y_e = 0.464; Y_e < 0.48; Y_e = Y_e + 0.001) {
            ...
            * success_str = "";
            ...
            check_within_range(Sirius_B_m_lower, Sirius_B_m_upper, m_solar, &results_success_Sirius_1, &success_str, "\t*SIRIUS B MASS");
            check_within_range(Sirius_B_r_lower, Sirius_B_r_upper, r_solar, &results_success_Sirius_2, &success_str, "\t*SIRIUS B RADIUS");
            check_within_range(Eri_B_m_lower, Eri_B_m_upper, m_solar, &results_success_Eri_1, &success_str, "\t*ERI B MASS");
            check_within_range(Eri_B_r_lower, Eri_B_r_upper, r_solar, &results_success_Eri_2, &success_str, "\t*ERI B RADIUS");
            ...
            if (results_success_Sirius_1 ||
                ...
                printf("\t%lf\t%lf\t%s", m_solar, r_solar, success_str);
